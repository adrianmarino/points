defmodule Mix.Tasks.Points.Client.Users do
  ...
  defmodule Show do
    ...
    defrun fn([token | [email | _]])-> points(base_url, token) |> users(:show, email: email) end
  end
  defmodule Create do
    use Mix.Task.Point.Client
    @shortdoc "Create a user. Params: token email password first_name last_name"
    defrun fn([token | user])-> points(base_url, token) |> users(:create, User.create(user)) end
  end
  defmodule Update do
    use Mix.Task.Point.Client
    @shortdoc "Update a user. Params: token email password first_name last_name"
    defrun fn([token | user])-> points(base_url, token) |> users(:update, User.create(user)) end
  end
  defmodule Delete do
    use Mix.Task.Point.Client
    @shortdoc "Delete a user. Params: token email"
    defrun fn([token | [email | _]])-> points(base_url, token) |> users(:delete, email: email) end
  end
end
