defmodule Mix.Tasks.Points.Client.ExchangeRates do
  ...
  defmodule Show do
    ...
    defrun fn([token | params]) ->
      points(base_url, token) |> exchange_rates(show: ExchangeRateId.new(params))
    end
  end
  defmodule Create do
    use Mix.Task.Point.Client
    ...
    defrun fn([token | params]) ->
      points(base_url, token) |> exchange_rates(create: ExchangeRate.new(params))
    end
  end
  defmodule Update do
    use Mix.Task.Point.Client
    ...
    defrun fn([token | params]) ->
      points(base_url, token) |> exchange_rates(update: ExchangeRate.new(params))
    end
  end
  defmodule Delete do
    use Mix.Task.Point.Client
    ...
    defrun fn([token | params]) ->
      points(base_url, token) |> exchange_rates(delete: ExchangeRateId.new(params))
    end
  end
end
