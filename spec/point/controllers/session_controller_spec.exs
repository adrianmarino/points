defmodule Point.SessionControllerSpec do
  ...
  let valid_attrs: %{email: "adrianmarino@gmail.com", password: "Whatever1123", first_name: "2222", last_name: "222"}
  let response: post(build_conn, session_path(build_conn, :create), user: user)
  ...
  context "when create a session with a valid password" do
    ...
    it "response a token that has a session assigned" do
      expect token = json_response(response, 201)["token"]
